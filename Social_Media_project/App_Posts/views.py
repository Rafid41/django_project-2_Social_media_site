...
def home(request):
    ...
    posts = Post.objects.filter(author__in=following_list.values_list('following')) # models.py following column er value author er shoman hole tar post chole ashbe

    liked_post =Like.objects.filter(user=request.user)
    liked_post_list= liked_post.values_list('post', flat=True)  # liked post gular primary_key store hbe)
