...
def login_page(request):
    ...
    return render(request, 'App_Login/login.html',context= {'title': 'Login . Social Connect', 'form':form})
...
def edit_profile(request):
    ...
    return render(request, 'App_Login/profile.html', context= {'form': form, 'title':'Edit Profile . Social Connect'})
...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            post = form.save(commit = False) # author set hynay, author ForeignKey, tai eta kora lage
...
def follow(request, username):
    ...
    follower_user = request.user     #follower user nijei

    # models.py e chk korbe, follwer and flollowing 'Follow' class er field
    # check korbe ei argument er moto kono value set kora ase kina
